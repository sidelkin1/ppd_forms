...
from sqlalchemy.sql.expression import Select, Subquery, ScalarSelect
...
def _select_last_report(
    ...
    return select(
        ...
        MonthlyReport.dat_rep >= date_from,
        MonthlyReport.dat_rep <= date_to,
        or_(
...
def _select_sum_value(rate: str) -> ScalarSelect:
    ...
    return select(
        getattr(stmt.c, f'{rate}_all')
    ).where(
        stmt.c.field == MonthlyReport.field,
        stmt.c.well_name == MonthlyReport.well_name,
        stmt.c.dat_rep == MonthlyReport.dat_rep,
    ).scalar_subquery().correlate(MonthlyReport)


def select_profile_report(
    date_from: date,
    date_to: date,
) -> Select:
    last_report = _select_last_report(date_from, date_to)
