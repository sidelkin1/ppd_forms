...
@pytest.fixture
def arq_dao(arq_redis: ArqRedis):
    return ArqDAO(arq_redis)
...
async def test_get_tasks_no_filter(
    ...
    data = resp.json()
    assert len(data) == 2
    assert responses[0].model_dump(exclude_none=True) in data
    assert responses[1].model_dump(exclude_none=True) in data
...
async def test_get_tasks_with_filter(
    ...
    data = resp.json()
    assert len(data) == 1
    assert data[0] == responses[0].model_dump(exclude_none=True)
