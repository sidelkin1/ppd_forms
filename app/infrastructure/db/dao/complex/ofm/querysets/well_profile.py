...
from app.infrastructure.db.dao.complex.ofm.querysets.common import (
    select_cids,
    select_description,
)
...
def _check_interval_intersections(model: type[Base]) -> ColumnElement:
    return or_(
        or_(
            between(model.top, GeophysStAbsorp.top, GeophysStAbsorp.bottom),
            between(model.base, GeophysStAbsorp.top, GeophysStAbsorp.bottom),
        ),
        or_(
            between(GeophysStAbsorp.top, model.top, model.base),
            between(GeophysStAbsorp.bottom, model.top, model.base),
        ),
    )
...
def _select_layer_perf(
    ...
    *where_args,
    ...
    return (
        select(_get_str_sum(model, schema, func_name, field))
        .where(_check_interval_intersections(model), *where_args)
        .scalar_subquery()
        .correlate(GeophysSt, GeophysStAbsorp)
    )
...
def select_well_profiles() -> Select:
    return select(
        ...
        func.coalesce(_select_cid_layers(), _select_perfs()).label(
            "cid_layer"
        ),
        ...
        GeophysSt.rec_date >= bindparam("date_from"),
        GeophysSt.rec_date <= bindparam("date_to"),
    )
